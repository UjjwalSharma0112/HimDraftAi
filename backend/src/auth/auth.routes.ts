  ...
  (req, res, next) => {
    ...
    passport.authenticate("google", { session: false }, (err: any, user: any, info: any) => {
      if (err) {
        ...
        res.redirect(`${clientUrl}/login?error=oauth_failed&details=${encodeURIComponent(err.message || "Unknown error")}`);
        return;
      }
      if (!user) {
        console.error("Google OAuth Failed: No User Profile. Info:", info);
        res.redirect(`${clientUrl}/login?error=no_user&details=${encodeURIComponent(info?.message || "User profile authentication failed")}`);
        return;
      }
